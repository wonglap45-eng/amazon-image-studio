...
- AI 策划 A+ 图片：支持 `普通A+ / 标准A+ / 高级A+ / 手机A+` 四套 A+ 模块编排，并生成逐模块英文生图提示词。
...
4. 如果选择 `A+ 图`，默认使用 `普通A+`，也可以切换为 `标准A+`、`高级A+` 或 `手机A+` 编排。
5. 在策划输入框中粘贴产品标题、五点描述、产品说明或品牌说明。
6. 在“参考图”区域上传产品实拍图、包装图或结构图。
7. 点击 `AI策划` 或 `AI策划A+`，生成逐张方案。
8. 在右侧选择要生成的图片位，例如 `MAIN`、`PT01`、`A+L01`、`A+S01`、`A+P01` 或 `A+M01`。
...
手机A+ 编排面向移动端短屏阅读，当前固定 5 张 4:3 图片：
